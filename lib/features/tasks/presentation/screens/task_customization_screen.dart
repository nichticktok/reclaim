...
class _TaskCustomizationScreenState extends State<TaskCustomizationScreen> {
  ...
  // Get attribute and colors (computed once)
  String get _attribute {
    return widget.presetTask.attribute.isNotEmpty 
        ? widget.presetTask.attribute 
        : AttributeUtils.determineAttribute(
            title: widget.presetTask.title,
            description: widget.presetTask.description,
            category: widget.presetTask.category,
          );
  }

  Color get _attributeColor => AttributeUtils.getAttributeColor(_attribute);
  Color get _lightColor => _attributeColor.withValues(alpha: 0.15);
  Color get _mediumColor => _attributeColor.withValues(alpha: 0.3);
  Color get _accentColor => _attributeColor.withValues(alpha: 0.5);
  ...
  void _handleSave() {
    final selectedDays = _getSelectedDayNumbers();
    if (selectedDays.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Select at least one day of the week'),
        ...
        title: Row(
          ...
          children: [
            ...
            Text(
              'Customize Task',
              style: TextStyle(
                color: _attributeColor.withValues(alpha: 0.9),
              ),
            ),
          ],
        ),
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // Task title preview with attribute color
              Container(
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          _attribute,
                          style: TextStyle(
                            color: _attributeColor.withValues(alpha: 0.8),
                            fontSize: 12,
            ...
            Expanded(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: _attributeColor,
                  padding: const EdgeInsets.symmetric(vertical: 16),
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(12),
                  ),
      ...
      children: amounts.map((amount) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isSelected ? _accentColor : const Color(0xFF2A2A2A),
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: isSelected ? _attributeColor : Colors.white.withValues(alpha: 0.1),
                width: 1,
              ),
            ),
            child: Text(
  ...
  List<int> _getSelectedDayNumbers() {
    final days = <int>[];
    for (var i = 0; i < _selectedDays.length; i++) {
      if (_selectedDays[i]) {
        days.add(i + 1); // Monday = 1
      }
    }
    return days;
  }
